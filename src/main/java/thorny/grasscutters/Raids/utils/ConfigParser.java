...
public final class ConfigParser {
    ...
    private final String configPath = Grasscutter.getConfig().folderStructure.plugins + "Raids";
    ...
    public void loadConfig() {
        try (FileReader file = new FileReader(this.configFile)) {
            ...
            Grasscutter.getLogger().info("[Raids] Config Loaded!");
        } catch (Exception e) {
            this.config = new Config();
            this.config.setDefault();
            Grasscutter.getLogger().info("[Raids] Basic config creating...");
        }

        if (!saveConfig()) {
            Grasscutter.getLogger().error("[Raids] Unable to save config file.");
        }

        Grasscutter.getLogger().info("[Raids] Plugin loaded!");
    }
